...
class TypingSpeedGUI:
    def __init__(self):
        ...
        self.sample_label = tk.Label(
            self.frame, text=random.choice(self.texts), font=self.default_font
        )
        self.sample_label.grid(row=0, column=0, columnspan=2, padx=5, pady=10)

        self.input = tk.Entry(self.frame, width=40, font=self.input_font)
        ...
        self.reset_btn = tk.Button(
            self.frame, text="Reset", command=self.reset, font=self.default_font
        )
    ...
    def start(self, event):
        if not self.running:
            if not event.keycode in [16, 17, 18]:  # Shift, alt and ctrl key
                self.running = True
        ...
        if self.input.get() == self.sample_label.cget("text")[:-1]:
            self.running = False
